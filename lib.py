...
def inject(args) -> tuple[Config, bool]:
    ...
    if host and (real_host := cfg.resolve_direct(host)):
        ...
        # A direct state indicates a non-routed configuration.
        if host == real_host:
            args.state = state = 'd:'
            if args.last_state == state:
                return c, True
        else:
            args.state = state = 'd:' + host
            if args.last_state == state:
                return c, True
            c.attach(host, real_host)
    else:
        if args.output_file:
            # A routed state records the routing context.
            args.state = state = cfg.get_state().strip()
            if args.last_state == state:
                return c, True

        cfg.route(host).inject(c)
